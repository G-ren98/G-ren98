...
class AuroraBackground(QtWidgets.QWidget):
    ...
    def _tick(self):
        self._hue = (self._hue + 0.35 * self._speed_scale) % 360
        self.update()
...
class TitleBar(QtWidgets.QWidget):
    ...
    def __init__(self, title: str, parent=None):
        ...
        self.btn_settings.setCursor(QtCore.Qt.CursorShape.PointingHandCursor)
        self.btn_settings.setStyleSheet(
            "QPushButton { border: none; background: transparent; color: rgba(235,240,245,0.88); }"
            "QPushButton:hover { background: rgba(255,255,255,0.10); border-radius: 8px; }"
        )
        layout.addWidget(self.btn_settings)
        self.btn_settings.clicked.connect(self.settingsRequested.emit)

        # Window controls
        def make_btn(txt: str) -> QtWidgets.QPushButton:
...
class StatCard(GlassCard):
    def __init__(self, title: str, value: str, accent: QtGui.QColor, parent=None):
        ...
        self.progress = SoftProgress(accent)
...
class MiniLine(QtWidgets.QWidget):
    ...
    def set_speed_ms(self, ms: int):
        self.timer.setInterval(max(15, int(ms)))
...
class OverviewPage(QtWidgets.QWidget):
    def __init__(self):
        ...
        stats.setSpacing(12)
        self.stat_revenue = StatCard("Revenue", "$128,430", ACCENT_GREEN)
        self.stat_users = StatCard("Active Users", "24,891", ACCENT)
        self.stat_conv = StatCard("Conversion", "3.72%", ACCENT_ORANGE)
        stats.addWidget(self.stat_revenue, 1)
        stats.addWidget(self.stat_users, 1)
        stats.addWidget(self.stat_conv, 1)
        ...
        # Controls card
        controls = GlassCard()
        ctl = QtWidgets.QVBoxLayout(controls)
        ctl.setContentsMargins(18, 16, 18, 16)
        ct = QtWidgets.QLabel("Controls")
        ct.setFont(font_stack(12, QtGui.QFont.Weight.DemiBold))
        ct.setStyleSheet("color: rgba(235,240,245,0.86);")
        ctl.addWidget(ct)
        btn_pulse = NeonButton("Pulse Stat", ACCENT_GREEN)
        btn_pulse.clicked.connect(self._pulse_stat)
        ctl.addWidget(btn_pulse)
        spd_row = QtWidgets.QHBoxLayout()
        spd_row.addWidget(QtWidgets.QLabel("Chart Speed"))
        self.slider_speed = QtWidgets.QSlider(QtCore.Qt.Orientation.Horizontal)
        self.slider_speed.setRange(30, 200)
        self.slider_speed.setValue(70)
        self.slider_speed.valueChanged.connect(self.chart.set_speed_ms)
        spd_row.addWidget(self.slider_speed)
        ctl.addLayout(spd_row)
        color_row = QtWidgets.QHBoxLayout()
        color_row.addWidget(QtWidgets.QLabel("Accent"))
        btn_blue = NeonButton("Blue", ACCENT)
        btn_blue.clicked.connect(lambda: self.chart.set_color(ACCENT))
        btn_green = NeonButton("Teal", ACCENT_GREEN)
        btn_green.clicked.connect(lambda: self.chart.set_color(ACCENT_GREEN))
        btn_orange = NeonButton("Orange", ACCENT_ORANGE)
        btn_orange.clicked.connect(lambda: self.chart.set_color(ACCENT_ORANGE))
        color_row.addWidget(btn_blue)
        color_row.addWidget(btn_green)
        color_row.addWidget(btn_orange)
        ctl.addLayout(color_row)

        mid.addWidget(chart_card, 2)
        mid.addWidget(actions, 1)
        mid.addWidget(controls, 1)
        layout.addLayout(mid, 1)

    def _pulse_stat(self):
        self.stat_revenue.progress.set_value(random.uniform(0.55, 0.95))
...
class MessagesPage(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.search.setPlaceholderText("Search messages...")
    ...
    def _on_search(self, text: str):
        ...
        for w in self.rows:
            ...
            s = " ".join(lbl.text().lower() for lbl in labels)
            w.setVisible(t in s)


class SettingsPage(QtWidgets.QWidget):
    def __init__(self, bg: AuroraBackground):
        super().__init__()
        self.bg = bg
        layout = QtWidgets.QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        ...
        spd.addWidget(QtWidgets.QLabel("Background Speed"))
        s = QtWidgets.QSlider(QtCore.Qt.Orientation.Horizontal)
        s.setRange(0, 200)
        s.setValue(100)
        s.valueChanged.connect(lambda v: self.bg.set_speed_scale(v / 100.0))
        spd.addWidget(s)
        c.addLayout(spd)
        # Pause toggle
        pause = QtWidgets.QCheckBox("Pause background")
        pause.toggled.connect(self.bg.set_paused)
        c.addWidget(pause)
        c.addSpacing(8)
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.page_messages = MessagesPage()
        self.page_settings = SettingsPage(self._bg)
        ...
        self.titlebar.settingsRequested.connect(lambda: self.pages.setCurrentIndex(3))
